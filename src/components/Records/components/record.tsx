    ...
    >
      <div className="tw-flex tw-items-center tw-gap-x-4">
        ...
        <p>
          ...
          {name} - <strong>Tipo: {type}</strong> {/* Aquí se agrega el campo type */}
        </p>
      </div>
      ...
      <div className="tw-flex tw-text-left tw-gap-x-4 tw-items-center">
        {isSelecting ? (
          <button
            onClick={() =>
              navigate(
                `${ROUTES.LOAD_MONEY}?type=${cardType}&card=${card}`
              )
            }
...
function AccountItem({ name, origin, cvu }: Account) {
  const navigate = useNavigate();
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(cvu).then(() => {
      ...
      console.error('Error al copiar el CVU: ', err);
    });
  };
