  ...
  useEffect(() => {
    if (userActivities.length > 0) {
      const parsedRecords = userActivities
        .filter((activity: Transaction) => {
          const transactionType = calculateTransacionType(activity.amount, activity.type);
          console.log('Transaction Type:', transactionType); // Verifica el tipo de transacción
          return transactionType === ActivityType.TRANSFER_OUT;
        })
        .map((activity: Transaction) => {
          const parsed = parseRecordContent(
            { cvu: activity.cvu },
            RecordVariant.ACCOUNT
          );
          console.log('Parsed Record:', parsed); // Verifica el contenido parseado
          return parsed;
        });
      ...
      const uniqueRecords = parsedRecords.filter(
        (record, index, self) =>
          index === self.findIndex(
            (t) => (t.content as Transaction).cvu === (record.content as Transaction).cvu
          )
      );
  
      console.log('Unique Records:', uniqueRecords); // Verifica los registros únicos
      ...
      if (user && user.id) {
        createTransferActivity(
          ...
          Swal.fire({
            ...
            confirmButtonColor: 'rgb(138, 173, 219)',
            background: 'rgb(13, 27, 46)',
            color: '#fff',
          });
          navigate(`${ROUTES.HOME}`);
        }).catch((error) => {
          console.error('Error durante la transferencia:', error);
          console.log('Código de estado del error:', error.status); // Agregar esto
          // Aquí puedes manejar el error dependiendo del código de estado
          if (error.status === 410) {
            Swal.fire({
              icon: 'error',
              title: 'Error',
            ...
            alert('Ocurrió un error durante la transferencia: ' + error.message);
          }
        });
      }
    };
